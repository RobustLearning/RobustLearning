...
def loss_cores(epoch, y, t, ind, loss_all, loss_div_all, noise_prior):
    beta = f_beta(epoch)
    # if epoch == 1:
    ...
    loss_ = -torch.log(F.softmax(y,dim=0) + 1e-8)
    # sel metric
    ...
    loss_all[ind, epoch] = torch.tensor(loss_numpy).cuda()
    loss_div_all[ind, epoch] = torch.tensor(loss_div_numpy).cuda()
...
def f_beta(epoch):
    ...
    beta2 = np.linspace(0.0, 2, num=20)
    beta3 = np.linspace(2, 2, num=20)
