...
def train(model, device, loader, optimizer, epoch):
    ...
    for data, target in loader:
        ...
        pred=pred.cpu().clone().numpy().tolist()
        target=target.cpu().clone().numpy().tolist()
        for i in range(len(pred)):
            true.append(target[i])
            prediction.append(pred[i])
    train_loss /= len(loader.dataset)
    # print('Epoch: {}\nTraining loss: {:.4f}, Training accuracy: {}/{} ({:.2f}%)'.format(
    #     epoch,train_loss, correct, len(loader.dataset), 100. * correct / len(loader.dataset)))
    return prediction,true


# SEAL training
def train_soft(model, device, loader, optimizer, epoch):
    model.train()
    train_loss = 0
    correct = 0
    true = []
...
def train_ct(args, model1, model2, device, loader, optimizer1, optimizer2, epoch, p_keep):
    ...
    for data, target in loader:
        ...
        pred1, pred2 = output1.argmax(dim=1, keepdim=True), output2.argmax(dim=1,
                                                                           keepdim=True)  # get the index of the max log-probability
        correct1, correct2 = correct1 + pred1.eq(target.view_as(pred1)).sum().item(), correct2 + pred2.eq(
            target.view_as(pred2)).sum().item()
...
# normal testing
def test(model, device, loader, top5=False):
    model.eval()
    test_loss = 0
    correct = 0
    correct_k = 0
    ...
    with torch.no_grad():
        for data, target in loader:
            ...
            targets.append(target.cpu().clone().numpy().tolist())
            if top5:
    ...
    for i in range(len(preds)):
        for j in range(len(preds[i])):
            prediction.append(preds[i][j][0])
            true.append(targets[i][j])
    prediction=np.array(prediction)
    true=np.array(true)
    print("shaope:",prediction.shape)
    f1=f1_score(true,prediction,zero_division=0)
    prescision=precision_score(true,prediction,zero_division=0)
    recall=recall_score(true,prediction,zero_division=0)
    test_loss /= len(loader.dataset)
    if top5:
        print(
            'Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%), Top5 Testing accuracy: {}/{} [{:.2f}%]\n'.format(
                test_loss, correct, len(loader.dataset), 100. * correct / len(loader.dataset), correct_k,
                len(loader.dataset), 100. * correct_k / len(loader.dataset)))
    else:
        print('Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%)\n'.format(
            test_loss, correct, len(loader.dataset), 100. * correct / len(loader.dataset)))
    return correct/len(loader.dataset),f1,prescision,recall,prediction,true
...
def val_test(args, model, device, val_loader, test_loader, best_val_acc, save_path):
    ...
    print(
        'Val loss: {:.4f}, Testing loss: {:.4f}, Val accuracy: {:.2f}%, Best Val accuracy: {:.2f}%, Testing accuracy: {:.2f}%\n'.format(
...
def test_ct(args, model1, model2, device, loader, top5=False):
    ...
    with torch.no_grad():
        for data, target in loader:
            data, target = data.to(device), target.to(device)
            output1, output2 = model1(data), model2(data)
            test_loss1, test_loss2 = test_loss1 + F.nll_loss(output1, target,
                                                             reduction='sum').item(), test_loss2 + F.nll_loss(output2,
                                                                                                              target,
    ...
    if top5:
        print(
            'Model1 Testing. Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%), Top5 Testing accuracy: {}/{} [{:.2f}%]\nModel2 Testing. Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%), Top5 Testing accuracy: {}/{} [{:.2f}%]\n'.format(
                test_loss1, correct1, len(loader.dataset), 100. * correct1 / len(loader.dataset), correct1_k,
                len(loader.dataset), 100. * correct1_k / len(loader.dataset),
                test_loss2, correct2, len(loader.dataset), 100. * correct2 / len(loader.dataset), correct2_k,
                len(loader.dataset), 100. * correct2_k / len(loader.dataset)))
    else:
        print(
            'Model1 Testing. Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%)\nModel2 Testing. Testing loss: {:.4f}, Testing accuracy: {}/{} ({:.2f}%)\n'.format(
